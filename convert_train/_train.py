...
def predict(rcf, bucket, prefix, df):
    ...
    while n <= cant_train:  # have to split df, if not is too big for sagemaker
        ...
        results = rcf_inference.predict(train_data.dep_delay.to_numpy().reshape(-1, 1))
...
def save_results(origin, bucket, prefix, filename, df_final):
    df_score_mean_std = (
    ...
    df_final1["anormal_day"] = df_final1.apply(
        lambda row: 1 if row["score"] > row["mean"] + 3 * row["std"] else "0", axis=1
    )
    grl.save_df(origin, bucket, prefix, filename, df_final1)
